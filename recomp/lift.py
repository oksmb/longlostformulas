...
class Lifter:
        ...
        while work:
            ...
            while True:
                ...
                if m == 'call':
                    ...
                    break
                pc = nxt
                if pc in seen and pc in leaders:
                    break
    ...
    def rd(self, ins, op, size=None):
        ...
        a = self.ea(ins, op)
        if size == 32:
            return f'(ta={a},(ta&3)===0?i32[ta>>2]:r32(ta))'
        if size == 16:
            return f'rs16({a})'
        return f'i8[{a}]'
    ...
    def wr(self, ins, op, val, size=None):
        ...
        a = self.ea(ins, op)
        if size == 32:
            return f'{{const tv={val},tw={a};if((tw&3)===0)i32[tw>>2]=tv;else w32(tw,tv);}}'
        if size == 16:
            return f'w16({a},{val});'
        return f'u8[{a}]={val};'
    ...
    def lift_fpu(self, ins, out):
        ...
        if mod != 3:
            ...
            if opc in (0xd8, 0xdc, 0xda, 0xde):
                src = {0xd8: f'(ta={a},(ta&3)===0?f32[ta>>2]:rf32(ta))', 0xdc: f'rf64({a})', 0xda: f'r32({a})', 0xde: f'rs16({a})'}[opc]
                if reg in (2, 3):
                ...
                return True
            if opc == 0xd9:
                if reg == 0: out.append(f'ta={a};ftop=(ftop-1)&7;st[ftop]=(ta&3)===0?f32[ta>>2]:rf32(ta);')
                elif reg == 2: out.append(f'ta={a};if((ta&3)===0)f32[ta>>2]={ST(0)};else wf32(ta,{ST(0)});')
                elif reg == 3: out.append(f'ta={a};if((ta&3)===0)f32[ta>>2]={ST(0)};else wf32(ta,{ST(0)});ftop=(ftop+1)&7;')
    ...
    def lift_insn(self, ins, out):
        ...
        if m in ('add', 'sub', 'cmp', 'adc', 'sbb'):
            ...
            if m in ('adc', 'sbb'):
                ...
                return True
            if m == 'add':
                out.append(f'{{const a={a},b={b},r={sx("a+b", sz)};@C(cf=(r>>>0)<(a>>>0);@)@O(of=((a^r)&(b^r))<0;@)@R(fr=r;@){wr(ins, ops[0], "r")}}}')
            else:
                w = wr(ins, ops[0], "r") if m == 'sub' else ''
                out.append(f'{{const a={a},b={b},r={sx("a-b", sz)};@C(cf=(a>>>0)<(b>>>0);@)@O(of=((a^b)&(a^r))<0;@)@R(fr=r;@){w}}}')
            return True
        if m in ('and', 'or', 'xor', 'test'):
            sz = ops[0].size * 8
            o = {'and': '&', 'or': '|', 'xor': '^', 'test': '&'}[m]
            if m == 'xor' and ops[1].type == X.X86_OP_REG and ops[0].type == X.X86_OP_REG and ops[0].reg == ops[1].reg:
                out.append(f'@R(fr=0;@)@C(cf=false;@)@O(of=false;@){wr(ins, ops[0], "0")}'); return True
            a, b = rd(ins, ops[0]), rd(ins, ops[1], sz)
            if m == 'test':
                out.append(f'@R(fr={a}{o}{b};@)@C(cf=false;@)@O(of=false;@)'); return True
            out.append(f'{{const r={a}{o}{b};@R(fr=r;@)@C(cf=false;@)@O(of=false;@){wr(ins, ops[0], "r")}}}'); return True
        if m in ('inc', 'dec'):
            sz = ops[0].size * 8
            a = rd(ins, ops[0])
            if m == 'inc':
                out.append(f'{{const a={a},r={sx("a+1", sz)};@O(of=r<a;@)@R(fr=r;@){wr(ins, ops[0], "r")}}}')
            else:
                out.append(f'{{const a={a},r={sx("a-1", sz)};@O(of=r>a;@)@R(fr=r;@){wr(ins, ops[0], "r")}}}')
            return True
        if m == 'neg':
            sz = ops[0].size * 8
            out.append(f'{{const a={rd(ins, ops[0])},r={sx("-a", sz)};@C(cf=a!==0;@)@O(of=(a&r)<0;@)@R(fr=r;@){wr(ins, ops[0], "r")}}}'); return True
        if m == 'not':
    ...
    # ---------- traces ----------
    CCUSE = {'o': 'O', 'no': 'O', 'b': 'C', 'ae': 'C', 'e': 'R', 'ne': 'R', 'be': 'CR', 'a': 'CR', 's': 'R', 'ns': 'R', 'p': 'R', 'np': 'R', 'l': 'RO', 'ge': 'RO', 'le': 'RO', 'g': 'RO'}
    FULLDEF = {'add', 'sub', 'cmp', 'and', 'or', 'xor', 'test', 'neg'}
    MAXTRACE = int(__import__('os').environ.get('LF_MAXTRACE', '28'))
    FOLLOWCALL = __import__('os').environ.get('LF_FOLLOWCALL', '1') == '1'

    def flag_use(self, ins):
        m = ins.mnemonic
        for pre in ('set', 'cmov', 'j'):
            if m.startswith(pre) and m not in ('jmp', 'jecxz'):
                c = CCALIAS.get(m[len(pre):], m[len(pre):])
                if c in self.CCUSE: return self.CCUSE[c]
        if m in ('adc', 'sbb', 'rcl', 'rcr'): return 'C'
        if m in ('loope', 'loopne'): return 'R'
        if m in ('pushfd', 'sahf', 'lahf', 'into'): return 'CRO'
        return ''

    def lift_block(self, start):
        items = []          # (code, uses, fulldef)
        pc = start
        visited = set()
        n = 0
        while True:
            ins = self.decode(pc)
            if ins is None:
                items.append((f'return trap2({pc});', 'CRO', False)); break
            if pc in visited or n >= self.MAXTRACE:
                self.continuations.add(pc)
                items.append((f'return {pc};', 'CRO', False)); break
            visited.add(pc); n += 1
            nxt = pc + ins.size
            m = ins.mnemonic
            ops = ins.operands
            if m == 'call':
                if self.FOLLOWCALL and ops[0].type == X.X86_OP_IMM and self.in_text(ops[0].imm & 0xffffffff) and (ops[0].imm & 0xffffffff) not in visited and n < self.MAXTRACE - 8:
                    items.append((f'esp=(esp-4)|0;w32(esp,{nxt});', '', False)); pc = ops[0].imm & 0xffffffff; continue
                tgt = self.rd(ins, ops[0]) if ops[0].type != X.X86_OP_IMM else str(ops[0].imm & 0xffffffff)
                items.append((f'{{const t={tgt};esp=(esp-4)|0;w32(esp,{nxt});return t;}}', 'CRO', False)); break
            if m == 'ret':
                extra = ops[0].imm if ops else 0
                items.append((f'{{const t=r32(esp);esp=(esp+{4 + extra})|0;return t;}}', 'CRO', False)); break
            if m == 'jmp':
                if ops[0].type == X.X86_OP_IMM and self.in_text(ops[0].imm & 0xffffffff):
                    pc = ops[0].imm & 0xffffffff
                    continue
                tgt = self.rd(ins, ops[0]) if ops[0].type != X.X86_OP_IMM else str(ops[0].imm & 0xffffffff)
                items.append((f'return {tgt};', 'CRO', False)); break
            if m == 'jecxz':
                items.append((f'if(ecx===0)return {ops[0].imm & 0xffffffff};', 'CRO', False)); pc = nxt; continue
            if m in ('loop', 'loopne', 'loope'):
                c = {'loop': 'ecx!==0', 'loopne': '(ecx!==0&&fr!==0)', 'loope': '(ecx!==0&&fr===0)'}[m]
                items.append((f'ecx=(ecx-1)|0;if({c})return {ops[0].imm & 0xffffffff};', 'CRO', False)); pc = nxt; continue
            if m.startswith('j'):
                items.append((f'if({cc(m[1:])})return {ops[0].imm & 0xffffffff};', 'CRO', False)); pc = nxt; continue
            out = []
            try:
            ...
            if not ok:
                ...
                items.append((f'return trap2({pc});', 'CRO', False)); break
            items.append((''.join(out), self.flag_use(ins), m in self.FULLDEF or (m == 'xor')))
            pc = nxt
        self.last_visited = visited
        # backward liveness: drop flag computations nobody can observe
        live = set('CRO'); res = []
        for code, uses, full in reversed(items):
            for f in 'CRO':
                a, b = f'@{f}(', '@)'
                while a in code:
                    i = code.index(a); j = code.index(b, i)
                    code = code[:i] + (code[i + 3:j] if f in live else '') + code[j + 2:]
            if full: live -= set('CRO')
            live |= set(uses)
            res.append(code)
    ...
    def emit(self, path):
        ...
        work = sorted(a for a in self.leaders if self.in_text(a))
        while work:
            for a in work:
                done[a] = ''.join(self.lift_block(a))
            work = sorted(a for a in self.continuations if a not in done and self.in_text(a))
        self.leaders |= set(done)
        with open(path, 'w') as f:
            f.write('// generated by lift.py - do not edit\n')
            for a in sorted(done):
                f.write(f'function b{a:x}(){{{done[a]}}}\n')
